...
The supplied R Script (run_analyze.R) can be used to create the TidyData_mean_activity.txt, which is also part of this repository.

The tidy_data.txt in this repository was created by running the run_analysis.R script using R version 3.5.1 (2018-07-02) on Windows 1064-bit edition.
The script contains the following steps :
Reading the dataset from https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip , and downloading it to a local location if not already exist 
...
This script requires the dplyr (0.7.6) and tidyr (0.8.1) packages.
